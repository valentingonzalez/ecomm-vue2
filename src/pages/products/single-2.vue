        ...
        <div id="productDetails">
          <div class="d-flex flex-column flex-md-row justify-content-md-between align-items-md-center mb-3">
            ...
            <div class="align-self-end align-self-md-center mt-2 mt-sm-0 flex-shrink-0" v-else>
              <ShareDropdown :productTitle="product.title | styleTitle" :productDescription="product.description" :productImage="selectedImage" />
            </div>
          </div>
          <div class="product-section px-md-3 py-md-4">
            <div class="row align-items-stretch" itemscope itemtype="http://schema.org/Product">
              <div class="col-md-5 d-flex flex-column">
                ...
                <div class="row small-gutters d-none d-md-flex">
                  <div class="col-3 col-lg-2 mb-2" v-for="img in productImages" :key="img.id">
                    <div class="w-100 border p-1 cursor-pointer img-thumb" :class="{'selected' : selectedImage == img.url}" @click="selectImage(img.url)">
                      <img :src="img.url" alt="product image" class="w-100 h-100" style="object-fit: contain" />
                    </div>
                  </div>
                </div>
              </div>
              <div class="col-md-7 d-flex flex-column">
                <div>
                  ...
                  <!-- Product Title -->
                  <h2 v-if="product" itemprop="name" class="my-2 mt-md-0"><strong v-if="product.brand_name && !settings.products.hideBrandFromTitle">{{ brandName }}</strong> {{ product.title | styleTitle }}</h2>
                  ...
                  <div v-if="forRental && filteredRates" class="row small-gutters">
                    <div class="col-md-3 text-center d-flex flex-column" v-for="rate in filteredRates" :key="rate.header">
                      <div class="card mb-2">
                        ...
                        <div class="h5 font-weight-normal text-primary" :style="{ color: settings.priceColor || ''}">${{ rate.price }}</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="price-and-promo" v-else>
                  <div class="prices" v-if="!settings.products.hideReg && showPriceOutOfStock">
                    ...
                    <div v-if="!settings.products.hideReg  && !forRental" class="label d-flex">
                      ...
                      <div v-if="product.promo_price && !product.online_price" class="has-promo">
                        <div class="label">
                          {{ promoLabel }}
                        </div>
                      ...
                      <div :style="{ color: settings.colors.priceColor ? settings.colors.priceColor : ''}">
                        <div v-if="fPackQty" v-html="'Price listed is for a single item. Must be ordered in multiples of ' + fPackQty" />
                        <div v-else v-html="settings.products.regularPriceText || 'Regular Price'" />
                ...
                <div class="pt-4 pb-3 border-top border-gray">
                  <div class="d-flex">
                      <div v-if="(disableOrdering || !isQuantityTextVisible) && !isSpecial">
                      ...
                      <div v-if="isTruevalue" itemprop="availability">
                        <div class="label">
                          <template v-if="!isSpecial"><span v-html="inStockText" class="text-success" />, Available at</template>
                      ...
                      <div v-else>
                        <div v-if="!disableOrdering" class="d-flex align-items-center" itemprop="availability">
                          <svg v-if="specialOrderWarning" class="mr-2 shop-location" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
                            <g transform="translate(1 1)" fill="none" fill-rule="evenodd">
                              <path d="M10.124 8.998a2.046 2.046 0 01-1.942-2.043 2.045 2.045 0 11-4.091 0C4.09 8.085 3.175 9 2.045 9S0 8.147 0 6.545L3.273 0h9.818l3.273 6.545" stroke="#ED672F" stroke-width="1.636"/>
                              <path stroke="#ED672F" stroke-width="1.636" d="M2.455 9v7.364H9"/>
                              <path d="M18 10.636c0 2.537-4.09 6.628-4.09 6.628s-4.092-4.091-4.092-6.628c0-2.618 2.127-4.09 4.091-4.09 1.964 0 4.091 1.472 4.091 4.09z" stroke="#ED672F" stroke-width="1.636" stroke-linecap="square"/>
                              <circle fill="#ED672F" fill-rule="nonzero" cx="13.909" cy="10.636" r="1"/>
                            </g>
                          </svg>
                ...
                <div v-if="isQuantityTextVisible && !forRental" class="stock-text">
                  ...
                  <template v-else-if="locationInventory <= 0">
                    ...
                    <div class="d-flex align-items-center" v-if="specialOrdersDisallowed">
                      ...
                      <NotifyBackInStock v-if="!$root.$children[0].isAdmin" :productId="product.id" :productSku="product.sku" />
                    </div>
                    <div v-else class="mr-5" style="color: red;">
                      ...
                      <div v-if="settings.text.specialOrderText" class="my-2">
                        {{ settings.text.specialOrderText }}
                      </div>
                      <template v-if="settings.products.getQuoteSpecial && disableSpecialOrders">
                        <a :href="`mailto:${settings.products.getQuoteEmail}`" aria-label="Get Quote">
                  ...
                  <div v-if="settings.products.showPartnerInventories">
                    <a
                      ...
                      class="mb-2"
                      @click.prevent="
                        getPartnerProductInventoryBySku(product.sku)
                      "
                    >
                      <span>Check Inventory at other locations</span>
                    </a>

                    <b-modal
                      size="md"
                      class="modal-box"
                      ref="InventoryAvailableAt"
                      :lazy="false"
                      :visible="false"
                      hide-footer
                      title="Inventory at other locations"
                      @hidden="$refs.InventoryAvailableAt.hide()"
                    >
                      <div v-if="product.partner_business_product_inventory">
                        <div v-if="product.partner_business_product_inventory.length > 0">
                          <div
                            v-for="(
                              store, i
                            ) in product.partner_business_product_inventory"
                            :key="i"
                          >
                          <div class="mb-2 breadcrumb" v-if="store.num_inventory>0">
                          <b class="mr-2">{{ store.num_inventory }}</b> Available at
                            <a
                              class="ml-2"
                              @click.prevent="
                                switchStore({...store})
                  ...
                  <span v-if="!isTruevalue && product.extra && JSON.parse(product.extra).max_order_quantity">
                      Order limit:  <strong>{{JSON.parse(product.extra).max_order_quantity}}</strong>
                    </span>
                </div>
                ...
                <router-link v-else-if="product" :to="{ name: 'rental-form', params: {name: product.title, id: product.id}}" class="btn btn-primary text-center font-weight-bold">
                  Click to Request a Quote
                  <!-- <svg class="ml-2" width="17" height="13" xmlns="http://www.w3.org/2000/svg"><g transform="translate(-863 -467)" fill="none" fill-rule="evenodd"><rect stroke="#E9E9E9" fill="#FFF" x="135.5" y="117.5" width="1169" height="585" rx="5"/><g transform="translate(635 365)"><rect stroke="#E2E2E7" fill="#FFF" fill-rule="nonzero" x=".5" y=".5" width="645" height="144" rx="5"/><g transform="translate(17 89)"><rect fill="#1C8ACE" fill-rule="nonzero" width="243" height="40" rx="5"/><g stroke="#FFF" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="M212 19.5h15M222 14l5 5.5-5 5.5"/></g></g></g></g></svg> -->
                </router-link>
                ...
                <template v-if="!outOfStock && !hideFulfillmentOptions && isTruevalue">
                  ...
                  <div v-if="checkoutTypes.pickup" class="border-top">
                    ...
                    <div class="pb-3 d-flex align-items-center">
                      ...
                      <div v-if="isInStorePingConfirmed" class="font-weight-bold mr-3">Thank you for certifying you will be purchasing this product in-store! <button class="btn btn-primary btn-xs" @click="cancelInStorePing" :disabled="isCancellingInStorePing">Cancel</button><!-- We will send updates about this product to your inbox. --></div>
                      <!-- <div v-if="isInStorePingConfirmed && !isCustomer && !nonCustomerContactInfoSent" class="text-small">Thank you for letting us know! Please enter an email or phone to get updates:</div>
                      <input v-if="isInStorePingConfirmed && !isCustomer && !nonCustomerContactInfoSent" placeholder="Enter email or phone" v-model="nonCustomerContactInfo"><button @click="sendNonCustomerInfoForInStorePing">Send</button>
                      <div v-if="nonCustomerContactInfoSent" class="text-small">Thanks</div> -->
                    </div>
                  </div>
                  <div v-if="checkoutTypes.delivery && !product.settings.disableDelivery" class="py-3 d-flex align-items-center border-top">
                    <div>
                    ...
                    <div>
                      ...
                      <a @click.prevent="$bvModal.show('delivery-locations-modal')" href="#" v-if="deliveryLocations.length">View Delivery Area</a>
                  ...
                  <div v-else-if="product.settings.disableDelivery && product.settings.disableDeliveryText && product.settings.disableDeliveryText.length > 0">
                    <div class="font-weight-bold text-danger mr-5 mb-2">{{ product.settings.disableDeliveryText }}</div>
                  </div>
                  <div v-if="checkoutTypes.shipping" class="py-3 d-flex align-items-center border-top">
                    <div>
            ...
            <div class="row d-flex flex-column-reverse flex-lg-row align-items-stretch pt-4 mt-2" ref="videoPlayer">
              ...
              <div class="col-12" :class="{'col-lg-6': (productVideos && productVideos.length && settings.products.showVideos)}" v-if="settings.products.showDescription && product.description">
                <div class="border-top border-gray pt-4 pb-4">
                  ...
                  <p v-if="hasTextSetting(product, 'descriptionDisclaimer')" class="font-weight-bold"
                    v-html="product.settings.descriptionDisclaimer">
                  </p>
              ...
              <div class="border-top border-gray pt-4 pb-4" v-if="product.specs && product.specs.length > 0">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      isSpecial() {
        return this.product.location == '_eztv' || this.product.location == '_ezorg' || this.product.location == '_ezdib';;
      },
      fPackQty() {
      ...
      showOnlyXLeft() {
        return this.locationInventory >= 1 && this.locationInventory <= this.settings.products.inventoryItemsMin;
      },
      onlyXLeftText() {
      ...
      hasCustomQtyMessage() {
        ...
        return this.product.settings.qtyMessage && this.product.settings.qtyMessage.length > 0;
      },
      customQtyText() {
      ...
      hiddenPriceText() {
        const def = "Please contact us for price";
        return this.product.settings.hidePriceText || def;
      },
      ...
      promoLabel() {
        return this.settings.products.singlePromoLabel || 'Sale!';
      },
      promoQuantityHeader() {
    ...
    methods: {
      ...
      goToSizePage() {
        if (this.sizeSelected.slug && (this.product.sku !== this.sizeSelected.sku)) {
          console.log('gotosizepage', this.sizeSelected.slug, this.sizeSelected.sku);
          this.$router.push({
            name: "products-slug",
            params: {slug: this.sizeSelected.slug}
          });
        }
      },
      makeProductImages(images) {
        var newImages = [];
        var mainImg = { id: 1, url: this.product.image_url};
