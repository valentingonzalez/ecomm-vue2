    ...
    <div v-else>
      <div class="tabs justify-content-between">
        ...
        <div class="d-flex pull-right">
          <button class="btn-sm btn btn-outline-primary bg-white mt-2 text-nowrap" @click="downloadCsv" :disabled="isDownloading">
            <svg class="mr-3" width="20" height="20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M10 13.333v-10 10z" fill="currentColor"/><path d="M3.333 13.333v.834a2.5 2.5 0 002.5 2.5h8.334a2.5 2.5 0 002.5-2.5v-.834M13.333 10L10 13.333m0 0L6.667 10M10 13.333v-10" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
      ...
      <template v-if="products">
          <div :class="{'pe-none': loading}">
            ...
            <div class="d-flex mb-3 flex-xl-row flex-column">
              <div class="position-relative custom-control custom-switch custom-switch-sm" style="z-index: 10">
            ...
            <div v-if="products.length" class="d-flex justify-content-end mb-2">
              <div class="custom-control custom-switch my-xl-0 my-1">
                <input type="checkbox" v-model="compactView" class="custom-control-input" id="compactView-toggle">
                <label class="custom-control-label" for="compactView-toggle">
                   Compact view
                </label>
              </div>
            </div>
          ...
          <div class="position-relative d-report-panel" v-if="products.length">
            <div class="position-absolute d-flex justify-content-center w-100 mt-5" v-if="loading" style="z-index: 1">
              <div class="spinner-border"></div>
            </div>
            <transition-group name="list" tag="div" :duration="changingPrice ? 500 : 0">
              <DiscrepancyProduct :fromWizard="true" :ref="`product-${product.sku}`" :compactView="compactView" :selectable="bulkMode" @onChange="onChangePrice" @onSelect="onSelect" class="mb-3" v-for="product in products" :product="product" :key="product.sku" :loading="product.loading || false" :class="{ 'disabled' : loading, 'animate': changingPrice }" />
            </transition-group>
          </div>
          <div v-else>
  ...
  export default {
    ...
    data() {
      return {
        ...
        selectedLocation: this.$route.query.location_id || null,
    ...
    methods: {
      ...
      async save() {
        ...
        if(settings && settings.length)
          return await AdminService.processDiscrepancyReport({ settings: settings });
      },
      async onChangePrice(product) {
        let p = this.selectedProducts.find(e => e.sku == product.sku);
        let pd;
        if(p) pd = p.settings.find(e => e.discrepancyId == product.id);
        if(pd) pd.selection = product.selection;
        if(!this.$refs[`product-${product.sku}`][0].selected) {
          this.$refs[`product-${product.sku}`][0].selected = true;
          this.$refs[`product-${product.sku}`][0].onSelect();
        }
      },
      onLoadSortSelector(elm) {
        elm.selectOption(this.sortOptions.find(e => e.value == this.orderBy), true);
      },
      onSelectLocation(id, internal = false) {
        this.selectedLocation = id;
        if(!internal) this.resetFilters();
        this.$router.push({ query: { ...this.$route.query, location_id: id }}).catch(() => {});
      },
      onSort(val, internal) {
        this.orderBy = val;
      ...
      resetFilters() {
        ...
        this.resetAll = true;
        this.$router.push({ query: { location_id: this.selectedLocation} }).catch(() => {});
        this.resetAll = false;
        this.applyDiff = false;
        this.diff_op = '';
        this.diff_value = '';
