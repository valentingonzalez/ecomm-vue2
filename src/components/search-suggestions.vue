<template>
  ...
  >
      ...
      <div class="d-flex align-items-center cursor-pointer">
        ...
        <div v-if="suggestion.name == 'products'" class="text-capitalize d-flex align-items-center">
          <div class="avatar d-flex align-items-center justify-content-center mr-3">
            <img class="w-auto h-100" :src="suggestion.item.image_url || '/images/default-img.svg'" alt="Item Image" />
          </div>
          {{ suggestion.item.title }}
        </div>
        <div v-if="suggestion.name == 'brands'" class="text-capitalize d-flex align-items-center">
          <div class="avatar d-flex align-items-center justify-content-center mr-3">
            <img class="w-auto h-100" :src="suggestion.item.image" alt="Item Image" />
          </div>
    ...
    <template slot="after-suggestions" v-if="!hideFooter">
...
export default {
  ...
  methods: {
    getSuggestions(text) {
      if(this.debounceOnGetSuggestions === 0) {
        text = text || '""';
        this.$store.dispatch('searchSuggestion', text);
        this.$emit('onSuggestion', text);
      } else {
        const delay = parseInt(this.debounceOnGetSuggestions);
