<template>
    <div>
      <b-modal centered size="xl" ref="returnLabelModal" id="returnLabelModal" hide-footer hide-header
              ...
              <div class="col-md-8">
                <div class="sidebar">
                  <div class="p-4 h-100 d-flex flex-column">
                    <h4 class="mb-4">Create Return Shipping Label</h4>
                        ...
                        <form ref="toAddressForm" @submit.prevent="validateForm($event, toAddress)" :class="{ 'was-validated' : toAddress.validated, 'disabled': (toAddress.loading || validatedToAddress.loading) }" novalidate>
                          <div class="row">
                            <div class="col-md-12 form-group">
                              <label for="toaddress-firstname">Name</label>
                              <input v-model="toAddress.fields.first_name" id="toaddress-name" type="text" class="form-control" required/>
                            </div>
                        ...
                        <form ref="fromAddressForm"
                          ...
                          <div class="row">
                            <div class="col-md-6 form-group">
                              ...
                              <input v-model="fromAddress.fields.first_name" id="fromAddress-name" type="text" class="form-control" required/>
                            </div>
                            <div class="col-md-6 form-group">
                              <label for="toaddress-lastname">Last Name</label>
                              <input v-model="fromAddress.fields.last_name" id="fromAddress-lastname" type="text" class="form-control" required>
                            </div>
                            <div class="col-md-6 form-group">
  ...
  export default {
    name: 'returnLabelModal',
    components: {PrintLabelModal, VSelect, VueCtkDateTimePicker},
    props: ['orderId', 'show', 'orderAmount'],
    data() {
      return {
        addingCarrier: false,
    ...
    methods: {
      ...
      fetchOrderInfo() {
        let lastFilledDetails = window.localStorage.getItem(this.orderId + "_ReturnFilledDetails");
        if(lastFilledDetails) {
          ...
          OrdersService.getShippingInfo(this.orderId).then(resp => {
            ...
            //Filling To Address
            // Since this is return label, we will use business address as to address
            ...
            Object.keys(resp.data.order.businessAddress).map(key => {
              ...
              if (key == 'business_address') {
                ...
                resp.data.order.businessAddress.first_name = resp.data.order.businessAddress[key];
                resp.data.order.businessAddress.last_name = resp.data.order.businessAddress[key];
            ...
            this.toAddress.fields = {...this.toAddress.fields, ...resp.data.order.businessAddress};

            //Filling To Address
            
            resp.data.order.customerAddress.state = this.states.find(e => e.value == resp.data.order.customerAddress.state);
            this.fromAddress.fields = {...this.fromAddress.fields, ...resp.data.order.customerAddress};
            this.fromAddress.fields.street1 = this.fromAddress.fields.address;
            this.fromAddress.fields.street2 = this.fromAddress.fields.address2;
            this.fromAddress.fields.phone = this.fromAddress.fields.telephone;
            delete this.fromAddress.fields.address;
            delete this.fromAddress.fields.address2;
            delete this.fromAddress.fields.telephone;
      ...
      async buyShippingLabel() {
        ...
        this.buyLabel.returnLabel = true;
        await OrdersService.buyShippingLabel(this.buyLabel, this.orderId)
          .then(resp => {
            ...
            if (resp.status !== 'error') {
              ...
              let lastFilledDetails = window.localStorage.getItem(this.orderId+"_ReturnFilledDetails");
              if(lastFilledDetails) {
                window.localStorage.removeItem(this.orderId+"_ReturnFilledDetails");
                window.localStorage.removeItem(this.orderId+"_returnLabeltabIndex");
              }
      ...
      prevTab() {
        ...
        window.localStorage.setItem(this.orderId + "_returnLabeltabIndex", this.tabIndex);
      },
      nextTab() {
        this.tabIndex += 1;
        window.localStorage.setItem(this.orderId + "_returnLabeltabIndex", this.tabIndex);
        let FilledDetails = {
          ...
          "returnAddress" : this.fromAddress,
          "parcel" : this.parcel,
        };
        window.localStorage.setItem(this.orderId + "_ReturnFilledDetails", JSON.stringify(FilledDetails));
      ...
      hideModal() {
        this.$refs.returnLabelModal.hide();
      },
      showModal() {
        this.$refs.returnLabelModal.show();
        this.fetchOrderInfo();
      },
